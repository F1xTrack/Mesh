...
final class zaaa implements zaca {
    ...
    private zaaa(Context context, zabe zabeVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, Map map2, ClientSettings clientSettings, Api.AbstractClientBuilder abstractClientBuilder, Api.Client client, ArrayList arrayList, ArrayList arrayList2, Map map3, Map map4) {
        ...
        while (it.hasNext()) {
            c7119x8.put((Api.AnyClientKey) it.next(), this.zad);
        }
        Iterator it2 = map.keySet().iterator();
        while (it2.hasNext()) {
            c7119x8.put((Api.AnyClientKey) it2.next(), this.zae);
        }
        this.zaf = Collections.unmodifiableMap(c7119x8);
    }
    ...
    public static zaaa zag(Context context, zabe zabeVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, ClientSettings clientSettings, Map map2, Api.AbstractClientBuilder abstractClientBuilder, ArrayList arrayList) {
        ...
        for (Map.Entry entry : map.entrySet()) {
            ...
            if (client2.requiresSignIn()) {
                c7119x8.put((Api.AnyClientKey) entry.getKey(), client2);
            } else {
                c7119x82.put((Api.AnyClientKey) entry.getKey(), client2);
            }
        }
        Preconditions.checkState(!c7119x8.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        C7119x8 c7119x83 = new C7119x8();
        C7119x8 c7119x84 = new C7119x8();
        for (Api api : map2.keySet()) {
            Api.AnyClientKey anyClientKeyZab = api.zab();
            if (c7119x8.containsKey(anyClientKeyZab)) {
                c7119x83.put(api, (Boolean) map2.get(api));
            } else {
                if (!c7119x82.containsKey(anyClientKeyZab)) {
                    throw new IllegalStateException("Each API in the isOptionalMap must have a corresponding client in the clients map.");
                }
                c7119x84.put(api, (Boolean) map2.get(api));
            }
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (c7119x83.containsKey(zatVar.zaa)) {
                arrayList2.add(zatVar);
            } else {
                if (!c7119x84.containsKey(zatVar.zaa)) {
                    throw new IllegalStateException("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
                }
                arrayList3.add(zatVar);
            }
        }
        return new zaaa(context, zabeVar, lock, looper, googleApiAvailabilityLight, c7119x8, c7119x82, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, c7119x83, c7119x84);
    }
    ...
    private final PendingIntent zaz() {
        ...
        return PendingIntent.getActivity(this.zaa, System.identityHashCode(this.zab), client.getSignInIntent(), AbstractC10330mx1.f38003a | 134217728);
    }
    ...
    public final void zau() {
        ...
        try {
            ...
            if (zZax) {
                new HandlerC10458nx1(this.zac).post(new zav(this));
            } else {
                zaB();
            }
            this.zam.unlock();
