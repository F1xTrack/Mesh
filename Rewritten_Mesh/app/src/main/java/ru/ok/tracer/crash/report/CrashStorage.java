...
public final class CrashStorage {
    ...
    private static final Comparator<Thread> THREAD_MAIN_FIRST_COMPARATOR = new Comparator<Thread>() { // from class: ru.ok.tracer.crash.report.CrashStorage$Companion$THREAD_MAIN_FIRST_COMPARATOR$1
        ...
        public int compare(Thread t1, Thread t2) {
            if (O90.a(t1 != null ? t1.getName() : null, "main")) {
                return -1;
            }
            if (O90.a(t2 != null ? t2.getName() : null, "main")) {
                return 1;
            }
    ...
    static {
        final C4243hv c4243hv = new C4243hv(0, new Comparator() { // from class: ru.ok.tracer.crash.report.CrashStorage$special$$inlined$compareBy$1
            /* JADX WARN: Multi-variable type inference failed */
            @Override // java.util.Comparator
            public final int compare(T t, T t2) {
                return GT1.b(((Thread) t).getName(), ((Thread) t2).getName());
            }
        });
        final Comparator comparator = new Comparator() { // from class: ru.ok.tracer.crash.report.CrashStorage$special$$inlined$thenBy$1
            ...
            @Override // java.util.Comparator
            public final int compare(T t, T t2) {
                int iCompare = c4243hv.compare(t, t2);
                if (iCompare != 0) {
                    return iCompare;
                }
                Thread thread = (Thread) t;
                Thread thread2 = (Thread) t2;
                return GT1.b(thread != null ? Long.valueOf(thread.getId()) : 0, thread2 != null ? Long.valueOf(thread2.getId()) : 0);
            }
        };
        THREAD_COMPARATOR = new Comparator() { // from class: ru.ok.tracer.crash.report.CrashStorage$special$$inlined$thenBy$2
            ...
            public final int compare(T t, T t2) {
                ...
                return iCompare != 0 ? iCompare : GT1.b(Integer.valueOf(System.identityHashCode(t)), Integer.valueOf(System.identityHashCode(t2)));
            }
        };
    }

    public CrashStorage(Context context) {
        O90.f(context, "appContext");
    ...
    private final CrashDescription read(File crashDir) throws Exception {
        try {
            ...
            O90.e(name, "crashDirName");
            int iF = AbstractC7538v51.F(name, '_', 0, 6);
            if (iF < 0) {
            ...
            String strSubstring2 = name.substring(iF + 1);
            O90.e(strSubstring2, "this as java.lang.String).substring(startIndex)");
            long j = Long.parseLong(strSubstring2);
            File fileG = AbstractC5702lU.g(crashDir, FILE_SYSTEM_INFO);
            if (!fileG.exists()) {
                throw new IllegalStateException("No system info file");
            }
            File fileG2 = AbstractC5702lU.g(crashDir, FILE_STACKTRACE);
            if (!fileG2.exists()) {
                throw new IllegalStateException("No stacktrace file");
            }
            File fileG3 = AbstractC5702lU.g(crashDir, FILE_TAGS);
            File fileG4 = AbstractC5702lU.g(crashDir, FILE_ALL_STACKTRACES);
            File fileG5 = AbstractC5702lU.g(crashDir, FILE_LOGS);
            String path = crashDir.getPath();
            O90.e(path, "crashDir.path");
            String path2 = fileG.getPath();
            O90.e(path2, "systemInfoFile.path");
            String path3 = fileG3.getPath();
            O90.e(path3, "tagsFile.path");
            String path4 = fileG2.getPath();
            O90.e(path4, "stacktraceFile.path");
            String path5 = fileG4.getPath();
            O90.e(path5, "allStacktracesFile.path");
            String path6 = fileG5.getPath();
            O90.e(path6, "logsFile.path");
            return new CrashDescription(j, crashTypeValueOf, path, path2, path3, path4, path5, path6);
        } catch (Exception e) {
            AbstractC5702lU.c(crashDir);
            throw e;
        }
    }
    ...
    public static /* synthetic */ CrashDescription save$default(CrashStorage crashStorage, CrashType crashType, Throwable th, SystemState systemState, List list, Map map, List list2, int i, Object obj) {
        int i2 = i & 8;
        MN mn = MN.a;
        return crashStorage.save(crashType, th, systemState, (List<String>) (i2 != 0 ? mn : list), (Map<Thread, StackTraceElement[]>) ((i & 16) != 0 ? NN.a : map), (List<LogEntry>) ((i & 32) != 0 ? mn : list2));
    }

    public final void deleteAll() {
        File fileG = AbstractC5702lU.g(TracerFiles.INSTANCE.getTracerDir(this.appContext), DIR_CRASHES);
        if (fileG.exists() && !AbstractC5702lU.c(fileG)) {
    ...
    public final List<CrashDescription> readAll() {
        ...
        if (arrayList.size() > 1) {
            AbstractC0236Cu.l(arrayList, new Comparator() { // from class: ru.ok.tracer.crash.report.CrashStorage$readAll$$inlined$sortBy$1
                /* JADX WARN: Multi-variable type inference failed */
                @Override // java.util.Comparator
                public final int compare(T t, T t2) {
                    return GT1.b(Long.valueOf(((CrashDescription) t).getTimestamp()), Long.valueOf(((CrashDescription) t2).getTimestamp()));
                }
            });
        }
        ...
        while (((CrashDescription) AbstractC8069xu.A(arrayList)).getTimestamp() < jCurrentTimeMillis) {
            if (arrayList.isEmpty()) {
                throw new NoSuchElementException("List is empty.");
            }
    ...
    public final CrashDescription save(CrashType type, Throwable throwable, SystemState systemState, List<String> list, Map<Thread, StackTraceElement[]> allStackTraces, List<LogEntry> logs) {
        O90.f(type, "type");
        O90.f(throwable, "throwable");
        O90.f(systemState, "systemState");
        O90.f(list, FILE_TAGS);
        O90.f(allStackTraces, "allStackTraces");
        O90.f(logs, "logs");
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(byteArrayOutputStream, AbstractC7680vr.a), 8192);
        try {
            ThrowableUtils.appendStackTraceTo(throwable, bufferedWriter);
            AbstractC2141aT1.a(bufferedWriter, null);
            byte[] byteArray = byteArrayOutputStream.toByteArray();
            O90.e(byteArray, "content.toByteArray()");
    ...
    public static /* synthetic */ CrashDescription save$default(CrashStorage crashStorage, CrashType crashType, byte[] bArr, SystemState systemState, List list, Map map, List list2, int i, Object obj) {
        int i2 = i & 8;
        MN mn = MN.a;
        return crashStorage.save(crashType, bArr, systemState, (List<String>) (i2 != 0 ? mn : list), (Map<Thread, StackTraceElement[]>) ((i & 16) != 0 ? NN.a : map), (List<LogEntry>) ((i & 32) != 0 ? mn : list2));
    }
    ...
    */
    public final ru.ok.tracer.crash.report.CrashDescription save(ru.ok.tracer.crash.report.CrashType r23, byte[] r24, ru.ok.tracer.SystemState r25, java.util.List<java.lang.String> r26, java.util.Map<java.lang.Thread, java.lang.StackTraceElement[]> r27, java.util.List<ru.ok.tracer.crash.report.LogEntry> r28) {
        /*
            Method dump skipped, instructions count: 548
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: ru.ok.tracer.crash.report.CrashStorage.save(ru.ok.tracer.crash.report.CrashType, byte[], ru.ok.tracer.SystemState, java.util.List, java.util.Map, java.util.List):ru.ok.tracer.crash.report.CrashDescription");
    }
}
