...
public class CryptoAlgoGcm implements CryptoAlgo {
    ...
    public int getCipherMetaDataLength() {
        CryptoConfig cryptoConfig = this.mConfig;
        return cryptoConfig.ivLength + 2 + cryptoConfig.tagLength;
    }

    @Override // com.facebook.crypto.CryptoAlgo
    public OutputStream wrap(OutputStream outputStream, Entity entity, byte[] bArr) throws CryptoInitializationException, IOException, KeyChainException {
    ...
    @Override // com.facebook.crypto.CryptoAlgo
    public InputStream wrap(InputStream inputStream, Entity entity) throws CryptoInitializationException, IOException, KeyChainException {
        byte b = (byte) inputStream.read();
        byte b2 = (byte) inputStream.read();
        Assertions.checkArgumentForIO(b == 1, "Unexpected crypto version " + ((int) b));
        Assertions.checkArgumentForIO(b2 == this.mConfig.cipherId, "Unexpected cipher ID " + ((int) b2));
