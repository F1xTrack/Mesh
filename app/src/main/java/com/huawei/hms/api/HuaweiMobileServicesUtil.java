...
public abstract class HuaweiMobileServicesUtil {
    ...
    private static String getHMSPackageName(Context context) {
        ...
        return TextUtils.isEmpty(hMSPackageName) ? PackageConstants.SERVICES_PACKAGE : hMSPackageName;
    }
    ...
    public static int isHuaweiMobileServicesAvailable(Context context, int i) {
        ...
        if (HMSPackageManager.getInstance(context).isUseOldCertificate()) {
            HMSLog.m12620e(TAG, "The CP uses the old certificate to terminate the connection.");
            return 13;
        }
        ...
        if (TextUtils.isEmpty(hMSPackageNameForMultiService)) {
            HMSLog.m12623w(TAG, "hmsPackageName is empty, Service is invalid.");
            return 1;
        }
        HMSLog.m12622i(TAG, "hmsPackageName is " + hMSPackageNameForMultiService);
        ...
        if (packageStates.equals(hMSPackageStatesForMultiService)) {
            HMSLog.m12623w(TAG, "HMS is not installed");
            return 1;
        }
        ...
        HMSLog.m12622i(TAG, "connect versionCode:" + HMSPackageManager.getInstance(context).getHmsMultiServiceVersion());
        if (!HMSPackageManager.getInstance(context).isApkUpdateNecessary(i)) {
            return 0;
        }
        HMSLog.m12622i(TAG, "The current version does not meet the minimum version requirements");
        return 2;
    }
