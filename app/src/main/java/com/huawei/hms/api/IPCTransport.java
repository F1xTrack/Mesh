    ...
    private int syncCall(ApiClient apiClient, IAIDLCallback iAIDLCallback) {
        if (!(apiClient instanceof InnerApiClient)) {
            if (apiClient instanceof AidlApiClient) {
                ...
                try {
                    ...
                    HMSLog.m12620e(TAG, "sync call ex:" + e);
                }
            }
        ...
        try {
            ...
            if (aidlApiClient2.getService() == null) {
                HMSLog.m12620e(TAG, "HuaweiApiClient is not binded to service yet.");
                return CommonCode.ErrorCode.INTERNAL_ERROR;
            }
            aidlApiClient2.getService().asyncCall(dataBuffer2, iAIDLCallback);
            return 0;
        } catch (Exception e2) {
            HMSLog.m12620e(TAG, "sync call ex:" + e2);
            return CommonCode.ErrorCode.INTERNAL_ERROR;
        }
    }

    @Override // com.huawei.hms.support.api.transport.DatagramTransport
    public final void post(ApiClient apiClient, DatagramTransport.InterfaceC2262a interfaceC2262a) {
        send(apiClient, interfaceC2262a);
    }

    @Override // com.huawei.hms.support.api.transport.DatagramTransport
    public final void send(ApiClient apiClient, DatagramTransport.InterfaceC2262a interfaceC2262a) {
        int iSyncCall = syncCall(apiClient, new IPCCallback(this.mResponseClass, interfaceC2262a));
        if (iSyncCall == 0 || interfaceC2262a == null) {
            return;
        }
        interfaceC2262a.mo12574a(iSyncCall, null);
    }
