...
public class InitReport {
    ...
    public static void enableConnectNet() {
        ...
        try {
            ...
            Logger.m11682e(TAG, "the thread submit has rejectedExecutionException!");
        } catch (Throwable unused2) {
            Logger.m11682e(TAG, "the thread submit has fatal error!");
        }
    }
    ...
    public static void reportWhenInit(Runnable runnable) {
        if (!hasConnectNet) {
            if (eventsToReport.size() > 10) {
                Logger.m11682e("TAG", "the event to be report when init exceed the limit!");
                return;
            } else {
    ...
    public static void submitAllEvents() {
        try {
            ...
            Logger.m11682e(TAG, "event is null occured");
        } catch (RejectedExecutionException unused2) {
            Logger.m11682e(TAG, "submit failed of rejected execution exception");
        } catch (Exception unused3) {
            Logger.m11682e(TAG, "submit failed because of some exception");
        }
    }
}
