...
public class HianalyticsHelper {
    ...
    private HianalyticsHelper() {
        try {
            ...
            Logger.m11685i(TAG, "Hianalytics sdk not found");
        ...
        Logger.m11688v(TAG, "this time the ha %s, mini %s", Boolean.valueOf(this.hasHianalytics), Boolean.valueOf(this.hasHMSBI));
    }
    ...
    private void onNewEvent(Context context, String str, Map map, int i) {
        ...
        Logger.m11688v(TAG, "data = %s", map);
        try {
            HiAnalyticsUtils.getInstance().onNewEvent(context, str, map, i);
        } catch (NoSuchMethodError unused) {
            Logger.m11689w(TAG, "may be you need upgrade stats sdk");
        } catch (Throwable unused2) {
            Logger.m11685i(TAG, "the stats has other error,pls check it");
        }
    }

    private void tryHMSBIInit(Context context) {
        if (context == null) {
            Logger.m11685i(TAG, "the appContext hasn't init");
            return;
        }
        try {
            ...
            Logger.m11689w(TAG, "maybe you need add base sdk!");
        } catch (Throwable unused2) {
            Logger.m11689w(TAG, "the hms base has other error!");
        }
    }
            ...
            try {
                this.reportExecutor.submit(new Runnable() { // from class: com.huawei.hms.framework.common.hianalytics.HianalyticsHelper.1
                    ...
                    public void run() {
                        ...
                        crashHianalyticsData.put(CrashHianalyticsData.CRASH_TYPE, Core.Event.Result.EXCEPTION);
                ...
                Logger.m11685i(TAG, "reportException error RejectedExecutionException");
            } catch (Exception unused2) {
                Logger.m11686i(TAG, "reportException error!", th);
            }
        }
    }
    ...
    public void setRate(int i) {
        ...
        this.bReportable = z;
        Logger.m11685i(TAG, "bReportable = " + this.bReportable + ", inuser = " + this.bInstallWelink + ", rate = " + i);
    }
    ...
    private boolean isEnableReport(Context context, boolean z, boolean z2) {
        ...
        try {
            ...
            Logger.m11689w(TAG, "the setting has illegalStateException");
        } catch (Throwable unused2) {
            Logger.m11689w(TAG, "the setting has other error");
        }
        Logger.m11685i(TAG, "user experience involved needs to be opened");
        return false;
    }
    ...
    public void onEvent(LinkedHashMap<String, String> linkedHashMap, String str, int i) {
        ...
        Logger.m11688v(TAG, "data = %s", linkedHashMap);
        ...
        if (this.hasHMSBI) {
            ...
            Logger.m11688v(TAG, "the base sdk isn't exsit, and reportType is %s", Integer.valueOf(i));
            return;
        }
        if (this.hasHianalytics) {
            ...
            if (hiAnalyticsInstance != null) {
                ...
                Logger.m11682e(TAG, "the ha has error,has init but is null!");
            }
        }
    }
