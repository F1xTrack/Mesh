...
public class ProviderCheckUtil {
    ...
    private static boolean checkSignaturesMatch(PackageManager packageManager, String str) {
        if (packageManager.checkSignatures(PackageConstants.SERVICES_PACKAGE, str) == 0) {
            Logger.m11687v(TAG, "Valid Provider");
            return true;
        }
        if (packageManager.checkSignatures("com.huawei.hwid.tv", str) != 0) {
            return false;
        }
        Logger.m11687v(TAG, "Valid Provider in tv");
        return true;
    }
    ...
    public static boolean isValid(Uri uri) {
        ...
        if (providerInfoResolveContentProvider == null || (applicationInfo = providerInfoResolveContentProvider.applicationInfo) == null) {
            Logger.m11689w(TAG, "Invalid param");
            return false;
        }
        String str = applicationInfo.packageName;
        Logger.m11687v(TAG, "Target provider service's package name is : " + str);
        if (str != null) {
            return checkSignaturesMatch(packageManager, str);
        }
        Logger.m11689w(TAG, "Invalid packageName");
        return false;
    }
}
