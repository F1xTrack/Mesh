...
public class AssetsUtil {
    ...
    public static String[] list(final Context context, final String str) {
        ...
        try {
            ...
            Logger.m11689w(TAG, "get local config files from sp task timed out");
            return new String[0];
        } catch (Exception unused2) {
            Logger.m11689w(TAG, "get local config files from sp task occur unknown Exception");
            return new String[0];
        } catch (InterruptedException e) {
            Logger.m11690w(TAG, "get local config files from sp task interrupted", e);
            return new String[0];
        } catch (ExecutionException e2) {
            Logger.m11690w(TAG, "get local config files from sp task failed", e2);
            return new String[0];
        } finally {
    ...
    public static InputStream open(Context context, String str) throws IOException {
        ...
        try {
            ...
            Logger.m11683e(TAG, "AssetManager has been destroyed", e);
            return null;
        }
    }
}
