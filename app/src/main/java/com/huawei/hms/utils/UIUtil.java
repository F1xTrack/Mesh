...
public class UIUtil {
    /* renamed from: a */
    private static int m12716a(Context context) {
        if (context == null) {
            return 0;
        }
    ...
    public static Activity getActiveActivity(Activity activity, Context context) {
        if (isBackground(context)) {
            HMSLog.m12622i("UIUtil", "isBackground" + isBackground(context));
            return null;
        }
        if (activity == null) {
            HMSLog.m12622i("UIUtil", "activity is null");
            return ActivityMgr.INST.getCurrentActivity();
        }
        if (!activity.isFinishing()) {
            return activity;
        }
        HMSLog.m12622i("UIUtil", "activity isFinishing is " + activity.isFinishing());
        return ActivityMgr.INST.getCurrentActivity();
    }

    public static int getDialogThemeId(Activity activity) {
        if (m12716a(activity) != 0) {
            return 0;
        }
    ...
    public static boolean isActivityFullscreen(Activity activity) {
        ...
        HMSLog.m12623w("UIUtil", "activity is null");
        return false;
    }
    ...
    public static boolean isBackground(Context context) {
        ...
        for (ActivityManager.RunningAppProcessInfo runningAppProcessInfo : runningAppProcesses) {
            if (TextUtils.equals(runningAppProcessInfo.processName, processName)) {
                HMSLog.m12622i("UIUtil", "appProcess.importance is " + runningAppProcessInfo.importance);
                boolean z = runningAppProcessInfo.importance == 100;
                boolean zIsKeyguardLocked = keyguardManager.isKeyguardLocked();
                HMSLog.m12622i("UIUtil", "isForground is " + z + "***  isLockedState is " + zIsKeyguardLocked);
