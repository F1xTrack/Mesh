...
public final class CollectionUtils {
    ...
    public static <K, V> Map<K, V> mapOfKeyValueArrays(K[] kArr, V[] vArr) {
        ...
        if (length != length2) {
            throw new IllegalArgumentException(AbstractC7222ym.m26229f(length, "Key and values array lengths not equal: ", length2, " != "));
        }
        if (length == 0) {
    ...
    public static <T> Set<T> mutableSetOfWithSize(int i) {
        return i == 0 ? new C0135C8(0) : zzb(i, true);
    }
    ...
    private static Map zza(int i, boolean z) {
        return i <= 256 ? new C7119x8(i) : new HashMap(i, 1.0f);
    }

    private static Set zzb(int i, boolean z) {
        if (i <= (true != z ? 256 : 128)) {
            return new C0135C8(i);
        }
