package ru.p031ok.tracer.utils.config;
...
public final class ConfigStorage {
    ...
    public final void init$tracer_commons_release(InterfaceC6434mZ fileProvider) {
        O90.m5968f(fileProvider, "fileProvider");
    ...
    public final void setLimits(String featureName, String tag, Long globalShutdownMs, Long featureShutdownMs, Long tagShutdownMs) {
        ...
        ConfigStorageKt.putShutdownMs(c8896bn0, "system.shutdown.until.ts", globalShutdownMs);
        ConfigStorageKt.putShutdownMs(c8896bn0, "system." + featureName + ".shutdown.until.ts", featureShutdownMs);
        if (tag != null) {
            ConfigStorageKt.putShutdownMs(c8896bn0, "system." + featureName + '.' + tag + ".shutdown.until.ts", tagShutdownMs);
        }
        getSettingsStorage().putAll(c8896bn0.m10510b());
    }
}
