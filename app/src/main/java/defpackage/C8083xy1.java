...
public final class C8083xy1 extends GoogleApi implements ActivityRecognitionClient {
    public static final Api a = new Api("ActivityRecognition.API", new C4063gy1(), new Api.ClientKey());

    @Override // com.google.android.gms.location.ActivityRecognitionClient
    public final Task removeActivityTransitionUpdates(PendingIntent pendingIntent) {
        return doWrite(TaskApiCall.builder().run(new C2040Zx1(1, pendingIntent)).setMethodKey(2406).build());
    }
    ...
    public final Task removeSleepSegmentUpdates(PendingIntent pendingIntent) {
        return doWrite(TaskApiCall.builder().run(new C2040Zx1(0, pendingIntent)).setMethodKey(2411).build());
    }
    ...
    public final Task requestActivityTransitionUpdates(ActivityTransitionRequest activityTransitionRequest, PendingIntent pendingIntent) {
        ...
        return doWrite(TaskApiCall.builder().run(new C2192ak1(activityTransitionRequest, 4, pendingIntent)).setMethodKey(2405).build());
    }
    ...
    public final Task requestActivityUpdates(long j, PendingIntent pendingIntent) {
        ...
        return doWrite(TaskApiCall.builder().run(new C2192ak1(zzbVarZzb, 8, pendingIntent)).setMethodKey(2401).build());
    }
    ...
    public final Task requestSleepSegmentUpdates(PendingIntent pendingIntent, SleepSegmentRequest sleepSegmentRequest) {
        ...
        return doRead(TaskApiCall.builder().run(new C1632Ur1(this, pendingIntent, sleepSegmentRequest)).setFeatures(zzm.zzb).setMethodKey(2410).build());
    }
}
