...
public final class C11896zC1 extends GoogleApi implements GeofencingClient {
    ...
    public final Task addGeofences(GeofencingRequest geofencingRequest, PendingIntent pendingIntent) {
        return doWrite(TaskApiCall.builder().run(new C8763ak1(geofencingRequest.zza(getContextAttributionTag()), 6, pendingIntent)).setMethodKey(2424).build());
    }

    @Override // com.google.android.gms.location.GeofencingClient
    public final Task removeGeofences(PendingIntent pendingIntent) {
        return doWrite(TaskApiCall.builder().run(new C10339n11(12, pendingIntent)).setMethodKey(2425).build());
    }
