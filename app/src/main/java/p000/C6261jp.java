package p000;

import com.facebook.react.bridge.Promise;
import com.mrousavy.camera.react.CameraViewModule;
import kotlin.jvm.functions.Function2;

/* renamed from: jp */
/* loaded from: classes2.dex */
public final class C6261jp extends AbstractC9714i71 implements Function2 {

    /* renamed from: a */
    public int f35412a;

    /* renamed from: b */
    public final /* synthetic */ CameraViewModule f35413b;

    /* renamed from: c */
    public final /* synthetic */ int f35414c;

    /* renamed from: d */
    public final /* synthetic */ Promise f35415d;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public C6261jp(int i, InterfaceC1382Vy interfaceC1382Vy, Promise promise, CameraViewModule cameraViewModule) {
        super(2, interfaceC1382Vy);
        this.f35413b = cameraViewModule;
        this.f35414c = i;
        this.f35415d = promise;
    }

    @Override // p000.AbstractC1300Uf
    public final InterfaceC1382Vy create(Object obj, InterfaceC1382Vy interfaceC1382Vy) {
        return new C6261jp(this.f35414c, interfaceC1382Vy, this.f35415d, this.f35413b);
    }

    @Override // kotlin.jvm.functions.Function2
    public final Object invoke(Object obj, Object obj2) {
        return ((C6261jp) create((InterfaceC0754Lz) obj, (InterfaceC1382Vy) obj2)).invokeSuspend(C8313Tf1.f11463a);
    }

    @Override // p000.AbstractC1300Uf
    public final Object invokeSuspend(Object obj) throws C10233mB0 {
        C0743Lo cameraSession$react_native_vision_camera_release;
        C9619hO0 c9619hO0;
        EnumC0817Mz enumC0817Mz = EnumC0817Mz.f7418a;
        int i = this.f35412a;
        if (i == 0) {
            RQ1.m7017d(obj);
            this.f35412a = 1;
            obj = this.f35413b.findCameraView(this.f35414c, this);
            if (obj == enumC0817Mz) {
                return enumC0817Mz;
            }
        } else {
            if (i != 1) {
                throw new IllegalStateException("call to 'resume' before 'invoke' with coroutine");
            }
            RQ1.m7017d(obj);
        }
        C4140gp c4140gp = (C4140gp) obj;
        Promise promise = this.f35415d;
        try {
            O90.m5968f(c4140gp, "<this>");
            cameraSession$react_native_vision_camera_release = c4140gp.getCameraSession$react_native_vision_camera_release();
            O90.m5968f(cameraSession$react_native_vision_camera_release, "<this>");
            cameraSession$react_native_vision_camera_release.f6874s = true;
            c9619hO0 = cameraSession$react_native_vision_camera_release.f6873r;
        } catch (Throwable th) {
            th.printStackTrace();
            AbstractC6386lo c10233mB0 = th instanceof AbstractC6386lo ? th : new C10233mB0(th);
            promise.reject(c10233mB0.f37292a + "/" + c10233mB0.f37293b, c10233mB0.f37294c, c10233mB0.getCause());
        }
        if (c9619hO0 == null) {
            throw new C6640po(27, false);
        }
        c9619hO0.close();
        cameraSession$react_native_vision_camera_release.f6873r = null;
        promise.resolve(null);
        return C8313Tf1.f11463a;
    }
}
