...
public class FlexBuffersBuilder {
    ...
    /* renamed from: bb */
    private final ReadWriteBuf f16150bb;
    ...
    private int align(int i) {
        int i2 = 1 << i;
        int iPaddingBytes = Value.paddingBytes(this.f16150bb.writePosition(), i2);
        while (true) {
            int i3 = iPaddingBytes - 1;
            if (iPaddingBytes == 0) {
                return i2;
            }
            this.f16150bb.put((byte) 0);
            iPaddingBytes = i3;
        }
    }
    ...
    private Value createKeyVector(int i, int i2) {
        ...
        while (i3 < this.stack.size()) {
            i3++;
            iMax = Math.max(iMax, Value.elemWidth(4, 0, this.stack.get(i3).key, this.f16150bb.writePosition(), i3));
        }
        int iAlign = align(iMax);
        writeInt(j, iAlign);
        int iWritePosition = this.f16150bb.writePosition();
    ...
    private Value createVector(int i, int i2, int i3, boolean z, boolean z2, Value value) {
        ...
        int iMax = Math.max(0, widthUInBits(j));
        if (value != null) {
            iMax = Math.max(iMax, value.elemWidth(this.f16150bb.writePosition(), 0));
            i4 = 3;
        } else {
            i4 = 1;
        }
        ...
        for (int i7 = i2; i7 < this.stack.size(); i7++) {
            iMax2 = Math.max(iMax2, this.stack.get(i7).elemWidth(this.f16150bb.writePosition(), i7 + i4));
            if (z && i7 == i2) {
        ...
        if (!z) {
            while (i8 < this.stack.size()) {
                this.f16150bb.put(this.stack.get(i8).storedPackedType(iMax2));
                i8++;
            }
        }
        if (value != null) {
    ...
    private int putKey(String str) {
        ...
        if ((this.flags & 1) == 0) {
            ...
            this.f16150bb.put(bytes, 0, bytes.length);
            this.f16150bb.put((byte) 0);
        ...
        this.f16150bb.put(bytes2, 0, bytes2.length);
        this.f16150bb.put((byte) 0);
        this.keyPool.put(str, Integer.valueOf(iWritePosition));
        return iWritePosition;
    }
    ...
    private Value writeBlob(int i, byte[] bArr, int i2, boolean z) {
        ...
        int iWritePosition = this.f16150bb.writePosition();
        this.f16150bb.put(bArr, 0, bArr.length);
        if (z) {
            this.f16150bb.put((byte) 0);
        }
    ...
    private void writeDouble(double d, int i) {
        if (i == 4) {
            this.f16150bb.putFloat((float) d);
        } else if (i == 8) {
            this.f16150bb.putDouble(d);
        }
    }

    private void writeInt(long j, int i) {
        if (i == 1) {
            this.f16150bb.put((byte) j);
            return;
        }
        if (i == 2) {
            this.f16150bb.putShort((short) j);
        } else if (i == 4) {
            this.f16150bb.putInt((int) j);
        } else {
            if (i != 8) {
                return;
            }
            this.f16150bb.putLong(j);
        }
    }

    private void writeOffset(long j, int i) {
        writeInt((int) (this.f16150bb.writePosition() - j), i);
    }
    ...
    public ByteBuffer finish() {
        int iAlign = align(this.stack.get(0).elemWidth(this.f16150bb.writePosition(), 0));
        writeAny(this.stack.get(0), iAlign);
        this.f16150bb.put(this.stack.get(0).storedPackedType());
        this.f16150bb.put((byte) iAlign);
        this.finished = true;
        return ByteBuffer.wrap(this.f16150bb.data(), 0, this.f16150bb.writePosition());
    }

    public ReadWriteBuf getBuffer() {
        return this.f16150bb;
    }
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i) {
        ...
        this.keyComparator = new Comparator<Value>() { // from class: androidx.emoji2.text.flatbuffer.FlexBuffersBuilder.1
            ...
            public int compare(Value value, Value value2) {
                ...
                int i3 = value2.key;
                do {
                    b = FlexBuffersBuilder.this.f16150bb.get(i2);
                    b2 = FlexBuffersBuilder.this.f16150bb.get(i3);
                    if (b == 0) {
                        return b - b2;
                    }
                    i2++;
